   ...
   , m_writeOnly(writeOnly)
{
   ...
   if (nMSAASamples == 1)
   {
      m_color_sampler = std::make_shared<Sampler>(m_rd, name + ".Color", m_type, m_color_tex, false);
      if (with_depth)
      {
         if (m_shared_depth)
            m_depth_sampler = sharedDepth->GetDepthSampler();
         else
            m_depth_sampler = std::make_shared<Sampler>(m_rd, name + ".Depth", m_type, m_depth_tex, false);
      }
   }
#ifndef __OPENGLES__
   ...
   else
   {
      ...
      m_color_sampler = std::make_shared<Sampler>(m_rd, name + ".Color", m_color_tex, false);
      if (with_depth && !m_shared_depth)
      {
         ...
         m_depth_sampler = std::make_shared<Sampler>(m_rd, name + ".Depth", m_depth_tex, false);
      }
   }
#endif
}
