...
// Signal handler for SIGSEGV to send fatal error event and capture stack trace
// Blocks crashed thread to allow Java error screen to be displayed
...
static std::atomic<int> s_crashSignal{0};
static sem_t s_crashSemaphore;
static sem_t s_userResponseSemaphore;  // Signaled when user responds to error screen
static pthread_t s_watchdogThread;

// Store old handlers to chain to them after user responds
static struct sigaction s_oldHandlers[32];
// Stack trace capture
static constexpr int MAX_BACKTRACE_FRAMES = 32;
static uintptr_t s_crashBacktrace[MAX_BACKTRACE_FRAMES];
static std::atomic<int> s_crashBacktraceSize{0};
...
static int capture_backtrace(uintptr_t* buffer, int max) {
    BacktraceState state = {buffer, buffer + max};
    _Unwind_Backtrace(unwind_callback, &state);
    return static_cast<int>(state.current - buffer);
}

// Watchdog thread - runs in background, waits for crash signal
static void* crash_watchdog_thread(void* arg)
{
   CRASH_LOG("Watchdog thread started, tid=%d", gettid());
   ...
   while (s_watchdogRunning.load())
   {
      ...
      if (sem_timedwait(&s_crashSemaphore, &ts) == 0)
      {
         int sig = s_crashSignal.load();
         CRASH_LOG("Watchdog detected crash signal %d, sending JNI callback", sig);

         // Format stack trace string
         char stackTrace[4096] = {0};
         char* ptr = stackTrace;
         int btSize = s_crashBacktraceSize.load();
         ptr += snprintf(ptr, 128, "Signal %d backtrace (%d frames):\n", sig, btSize);
         
         for (int i = 0; i < btSize && ptr < stackTrace + 3800; i++) {
            Dl_info info;
            if (dladdr((void*)s_crashBacktrace[i], &info)) {
               const char* symName = info.dli_sname ? info.dli_sname : "??";
               const char* libName = info.dli_fname ? info.dli_fname : "??";
               // Get just the library filename
               const char* libShort = strrchr(libName, '/');
               libShort = libShort ? libShort + 1 : libName;
               uintptr_t offset = s_crashBacktrace[i] - (uintptr_t)info.dli_fbase;
               ptr += snprintf(ptr, 256, "#%02d: %s (+0x%lx) [%s]\n", i, symName, (unsigned long)offset, libShort);
            } else {
               ptr += snprintf(ptr, 128, "#%02d: 0x%lx\n", i, (unsigned long)s_crashBacktrace[i]);
            }
         }
         ...
         VPinballLib::VPinballLib::SendEvent(VPINBALL_EVENT_FATAL_ERROR, &errorData);
   ...
   CRASH_LOG("Watchdog thread exiting");
   return nullptr;
}

// Signal handler using SA_SIGINFO to preserve crash context
static void vpinball_sigsegv_handler(int sig, siginfo_t* info, void* context)
{
   // Only handle once
   if (s_crashHandled.exchange(true))
   {
      // Already handled - just block this thread forever
      while (true) { pause(); }
      return;
   }
   ...
   s_crashSignal.store(sig);
   sem_post(&s_crashSemaphore);

   // Block this thread forever - let watchdog send events and Java show error screen
   // Do NOT chain to old handler - that would kill the process
   CRASH_LOG("Signal handler blocking - waiting for error screen to be shown");
   while (true) { pause(); }
}

static void install_crash_handler()
{
   if (!s_signalHandlerInstalled.exchange(true))
   {
      CRASH_LOG("Installing crash handler system...");
      sem_init(&s_crashSemaphore, 0, 0);
      s_watchdogRunning.store(true);
      pthread_create(&s_watchdogThread, nullptr, crash_watchdog_thread, nullptr);
...
{
   if (s_watchdogRunning.exchange(false))
   {
      CRASH_LOG("Cleaning up crash handler...");
      sem_post(&s_crashSemaphore);
      pthread_join(s_watchdogThread, nullptr);
      sem_destroy(&s_crashSemaphore);
      CRASH_LOG("Crash handler cleaned up");
   }
}
#endif
...
void RenderDevice::tBGFXCallback::fatal(const char* _filePath, uint16_t _line, bgfx::Fatal::Enum _code, const char* _str)
{
   ...
   // Instead of abort(), gracefully shut down the player so error can be reported
   // This allows the Android app to receive error info via Intent
#ifdef __LIBVPINBALL__
   // Send fatal error event with the error message
   std::string errorMsg = std::string(_filePath) + ":" + std::to_string(_line) + " - " + _str;
   VPinballLib::ScriptErrorData errorData = {
      VPINBALL_SCRIPT_ERROR_TYPE_RUNTIME,  // Use runtime error type for fatal errors
      static_cast<int>(_line),
      static_cast<int>(_code),  // Store bgfx error code in position field
      errorMsg
   };
   VPinballLib::VPinballLib::SendEvent(VPINBALL_EVENT_FATAL_ERROR, &errorData);

   // Request graceful player shutdown
   if (g_pplayer && g_pplayer->GetCloseState() != Player::CS_CLOSED)
   {
      PLOGE << "Requesting graceful player shutdown due to BGFX fatal error";
      g_pplayer->SetCloseState(Player::CS_STOP_PLAY);
   }

   // Send PLAYER_CLOSED event so app knows player is done
   VPinballLib::VPinballLib::SendEvent(VPINBALL_EVENT_PLAYER_CLOSED, nullptr);
#endif

   // Exit cleanly instead of abort() - this gives the app a chance to handle it
   // Use _exit to avoid calling atexit handlers which might cause more crashes
   _exit(1);
}

// Track if we've detected a pipeline error and are shutting down
static std::atomic<bool> s_pipelineErrorDetected{false};
...
void RenderDevice::tBGFXCallback::traceVargs(const char* _filePath, uint16_t _line, const char* _format, va_list _argList)
{
   ...
   if (!s_pipelineErrorDetected.load() &&
       (strstr(out, "Pipeline create failed") != nullptr ||
        strstr(out, "Failed to link shaders") != nullptr ||
        strstr(out, "Bind group info mismatches") != nullptr))
   {
      s_pipelineErrorDetected.store(true);
      PLOGE << "CRITICAL: Shader/Pipeline creation failed - triggering graceful shutdown to avoid crash";
      ...
      VPinballLib::ScriptErrorData errorData = {
         VPINBALL_SCRIPT_ERROR_TYPE_RUNTIME,
         static_cast<int>(_line),
         100,  // Custom error code for pipeline failure
         std::string(out)
      };
      VPinballLib::VPinballLib::SendEvent(VPINBALL_EVENT_FATAL_ERROR, &errorData);

      // Request graceful player shutdown before the crash happens
      if (g_pplayer && g_pplayer->GetCloseState() != Player::CS_CLOSED)
      {
         g_pplayer->SetCloseState(Player::CS_STOP_PLAY);
      }

      // Send PLAYER_CLOSED event
      VPinballLib::VPinballLib::SendEvent(VPINBALL_EVENT_PLAYER_CLOSED, nullptr);

      // Exit immediately to prevent the crash
      _exit(1);
#endif
   }
}
