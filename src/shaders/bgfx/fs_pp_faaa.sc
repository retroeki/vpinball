...
#ifdef STEREO
vec3 faaa(const vec2 u, const float v_eye)
#else
vec3 faaa(const vec2 u)
#endif
{
	const ARRAY_BEGIN(float, FAAAOffMult, 12) FAAA_QUALITY__P0, FAAA_QUALITY__P1,
		...
		FAAA_QUALITY__P10, FAAA_QUALITY__P11 ARRAY_END();
	...
	const vec2 dirM = vec2(abs(gradientSWNE + gradientSENW), abs(gradientSWNE - gradientSENW));
	...
	float lumaN, lumaS;
	if( horzSpan) lumaN = luma(texStereoNoLod(tex_fb_unfiltered, u - vec2(0.0,offs.y)).xyz);
	if( horzSpan) lumaS = luma(texStereoNoLod(tex_fb_unfiltered, u + vec2(0.0,offs.y)).xyz);
	if(!horzSpan) lumaN = luma(texStereoNoLod(tex_fb_unfiltered, u - vec2(offs.x,0.0)).xyz);
	if(!horzSpan) lumaS = luma(texStereoNoLod(tex_fb_unfiltered, u + vec2(offs.x,0.0)).xyz);
...
{
#ifdef STEREO
	gl_FragColor = vec4(faaa(v_texcoord0, v_eye), 1.0);
#else
	gl_FragColor = vec4(faaa(v_texcoord0), 1.0);
#endif
}
