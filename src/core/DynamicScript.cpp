...
bool DynamicTypeLibrary::COMToScriptVariant(const VARIANT* cv, const ScriptTypeNameDef& type, ScriptVariant& sv) const
{
   ...
   {
      VARTYPE vt = V_VT(cv);
      if (!((vt == (VT_ARRAY | VT_VARIANT)) || (vt == (VT_BYREF | VT_ARRAY | VT_VARIANT)))) {
         PLOGE.printf("DynamicScript: Array type mismatch! Expected VT_ARRAY|VT_VARIANT (0x%x) or VT_BYREF|VT_ARRAY|VT_VARIANT (0x%x), got 0x%x",
            VT_ARRAY | VT_VARIANT, VT_BYREF | VT_ARRAY | VT_VARIANT, vt);
         return false;
      }
      SAFEARRAY* psa = (vt & VT_BYREF) ? *V_ARRAYREF(cv) : V_ARRAY(cv);
