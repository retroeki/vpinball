      ...
      if (SUCCEEDED(hr = pstgData->OpenStream(L"GameData", nullptr, STGM_DIRECT | STGM_READ | STGM_SHARE_EXCLUSIVE, 0, &pstmGame)))
      {
         ...
         if (SUCCEEDED(hr = LoadData(pstmGame, loadfileversion, hch, (loadfileversion < NO_ENCRYPTION_FORMAT_VERSION) ? hkey : NULL)))
         {
            ...
            for (int i = 0; i < csubobj; i++)
            {
               ...
               {
                  PLOGI << "Loading cancelled during item processing";
                  hr = E_ABORT;
                  break;
               }
            ...
            // Check if loading was cancelled during item processing
            if (hr == E_ABORT || feedback.IsCancelled())
            {
               PLOGI << "Aborting load after items";
               hr = E_ABORT;
            }

            if (hr != E_ABORT) for (int i = 0; i < csounds; i++)
            {
               // Check for cancellation
               if (feedback.IsCancelled())
               {
                  PLOGI << "Loading cancelled during sound processing";
                  hr = E_ABORT;
                  break;
               }
            ...
            // Check if loading was cancelled during sound processing
            if (hr == E_ABORT || feedback.IsCancelled())
            {
               PLOGI << "Aborting load after sounds";
               hr = E_ABORT;
            }
            ...
            } // End of image loading block

            // Check if loading was cancelled during image processing
            if (hr != E_ABORT && feedback.IsCancelled())
            {
               PLOGI << "Aborting load after images";
               hr = E_ABORT;
            }

            if (hr != E_ABORT) for (int i = 0; i < cfonts; i++)
            {
               // Check for cancellation
               if (feedback.IsCancelled())
               {
                  PLOGI << "Loading cancelled during font processing";
                  hr = E_ABORT;
                  break;
               }
            ...
            // Check if loading was cancelled during font processing
            if (hr == E_ABORT || feedback.IsCancelled())
            {
               PLOGI << "Aborting load after fonts";
               hr = E_ABORT;
            }

            if (hr != E_ABORT) for (int i = 0; i < ccollection; i++)
            {
               // Check for cancellation
               if (feedback.IsCancelled())
               {
                  PLOGI << "Loading cancelled during collection processing";
                  hr = E_ABORT;
                  break;
               }
            ...
            // Check if loading was cancelled during collection processing
            if (hr == E_ABORT || feedback.IsCancelled())
            {
               PLOGI << "Aborting load after collections";
               hr = E_ABORT;
            }
         ...
         } // End of if (hr != E_ABORT) block after collections
      }
      pstgData->Release();
   }
...
{
   ViewSetupID previousMode = m_viewMode;
   if (m_viewModeOverride != BG_INVALID)
   {
      m_viewMode = m_viewModeOverride;
      PLOGI << "UpdateCurrentBGSet: override active, viewMode=" << (int)m_viewMode;
   }
   else
   {
...
STDMETHODIMP PinTable::get_ShowDT(VARIANT_BOOL *pVal)
{
   ...
   // On Android, always report fullscreen/cabinet mode to scripts
   // Some scripts run expensive desktop-mode code paths (extra lamp updates, timers)
   // that cause progressive slowdown on mobile devices
   *pVal = VARIANT_FALSE;
#else
