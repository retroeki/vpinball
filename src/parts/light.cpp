...
constexpr inline float clampLightState(const float state)
{
...
#define LinkProp(field, prop) field = fromMouseClick ? g_pvp->m_settings.GetDefaultPropsLight_##prop() : Settings::GetDefaultPropsLight_##prop##_Default()
   ...
   LinkProp(m_d.m_color2, ColorFull);
   LinkProp(m_d.m_szImage, OffImage);
   LinkProp(m_d.m_rgblinkpattern, BlinkPattern);
   LinkProp(m_d.m_blinkinterval, BlinkInterval);
   LinkProp(m_d.m_szSurface, Surface);
   LinkProp(m_d.m_fadeSpeedUp, FadeSpeedUp);
   LinkProp(m_d.m_fadeSpeedDown, FadeSpeedDown);
   LinkProp(m_d.m_intensity, Intensity);
   LinkProp(m_d.m_transmissionScale, TransmissionScale);
   LinkProp(m_d.m_BulbLight, Bulb);
   LinkProp(m_d.m_imageMode, ImageMode);
   LinkProp(m_d.m_showBulbMesh, ShowBulbMesh);
   LinkProp(m_d.m_staticBulbMesh, StaticBulbMesh);
   LinkProp(m_d.m_showReflectionOnBall, ShowReflectionOnBall);
   LinkProp(m_d.m_meshRadius, ScaleBulbMesh);
...
void Light::Render(const unsigned int renderMask)
{
   ...
   {
      // Log when segment lights are skipped due to decals disabled
      static int skipLogCount = 0;
      if (skipLogCount < 5 && wcsstr(m_wzName, L"seg") != nullptr) {
         skipLogCount++;
         string name = MakeString(m_wzName);
         PLOGI.printf("Light SKIP (backglass+no decals): name='%s' backglass=%d decals=%d",
            name.c_str(), m_backglass, GetPTable()->GetDecalsEnabled());
      }
      return;
   }
      ...
      && (m_lightmapMeshBuffer != nullptr)) // in case of degenerate light
   {
      ...
      // For backglass lights with zero intensity but non-black color (e.g. GLF segment displays),
      // use color brightness as fallback intensity so they render
      if (m_backglass && m_currentIntensity == 0.f && m_inPlayState != 0.f) {
         float colorBrightness = max(max(lightColor_intensity.x, lightColor_intensity.y), lightColor_intensity.z);
         if (colorBrightness > 0.f) {
            lightColor_intensity.w = colorBrightness; // Use color brightness as intensity
         }
      }
...
{
   double T = m_d.m_intensity != 0.f && m_d.m_intensity_scale != 0.f ? bulb_emission_to_filament_temperature(m_currentIntensity / (m_d.m_intensity * m_d.m_intensity_scale)) : 293.f; 
   *pVal = (float)T;
...
STDMETHODIMP Light::put_Visible(VARIANT_BOOL newVal)
{
   ...
   // Log when segment display lights are made visible (names like p1_seg, p2_seg, etc.)
   static int segLightLogCount = 0;
   if (segLightLogCount < 20 && m_wzName[0] == 'p' &&
       (m_wzName[1] == '1' || m_wzName[1] == '2' || m_wzName[1] == '3' || m_wzName[1] == '4' || m_wzName[1] == '_') &&
       wcsstr(m_wzName, L"seg") != nullptr) {
      segLightLogCount++;
      string name = MakeString(m_wzName);
      PLOGI.printf("Light::put_Visible: name='%s' visible=%s", name.c_str(), m_d.m_visible ? "True" : "False");
   }
   return S_OK;
}
