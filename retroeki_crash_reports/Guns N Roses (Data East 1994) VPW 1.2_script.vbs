...
Const SFlipperOff = ""
Const SCoin = ""
Dim UseVPMDMD : If 0 = 2 Then UseVPMDMD = True Else UseVPMDMD = Table1.ShowDT
...
Sub Table1_OptionEvent(ByVal eventId)
    ...
    ' Room Brightness
    If eventId <> 0 And LightLevel <> NightDay Then DisableStaticPreRendering = True
	LightLevel = NightDay
	...
	Dim ssp_i3
	If IsArray(BP_Post_L) Then
		For ssp_i3 = 0 To UBound(BP_Post_L)
			Set x = BP_Post_L(ssp_i3)
			x.Visible = v
		Next
	End If
	...
	Dim ssp_i4
	If IsArray(BP_Post_R) Then
		For ssp_i4 = 0 To UBound(BP_Post_R)
			Set x = BP_Post_R(ssp_i4)
			x.X = 940.0 + 13.0 * OutPostMod
		Next
	End If
	...
	Dim ssp_i5
	If IsArray(BP_BP_Empty) Then
		For ssp_i5 = 0 To UBound(BP_BP_Empty)
			Set x = BP_BP_Empty(ssp_i5)
			x.Visible = v
		Next
	End If
	Dim ssp_i6
	If IsArray(BP_BP_Jungle) Then
		For ssp_i6 = 0 To UBound(BP_BP_Jungle)
			Set x = BP_BP_Jungle(ssp_i6)
			x.Visible = 1 - v
		Next
	End If
	...
	Dim ssp_i7
	If IsArray(BP_Blades) Then
		For ssp_i7 = 0 To UBound(BP_Blades)
			Set x = BP_Blades(ssp_i7)
			x.Visible = v
		Next
	End If
...
'	Controller.Run
	...
	If 0 = 2 Then
		...
		Dim ssp_i8
		If IsArray(VRCabinet) Then
			For ssp_i8 = 0 To UBound(VRCabinet)
				Set VR_Obj = VRCabinet(ssp_i8)
				VR_Obj.Visible = 1
			Next
		End If
		Dim ssp_i9
		If IsArray(VRRoom) Then
			For ssp_i9 = 0 To UBound(VRRoom)
				Set VR_Obj = VRRoom(ssp_i9)
				VR_Obj.Visible = 1
			Next
		End If
		Dim ssp_i10
		If IsArray(VRBackglass) Then
			For ssp_i10 = 0 To UBound(VRBackglass)
				Set VR_Obj = VRBackglass(ssp_i10)
				VR_Obj.visible = 1
			Next
		End If
		'Set Up VR Backglass
	...
	Else
		...
		Dim ssp_i11
		If IsArray(VRCabinet) Then
			For ssp_i11 = 0 To UBound(VRCabinet)
				Set VR_Obj = VRCabinet(ssp_i11)
				VR_Obj.Visible = 0
			Next
		End If
		Dim ssp_i12
		If IsArray(VRRoom) Then
			For ssp_i12 = 0 To UBound(VRRoom)
				Set VR_Obj = VRRoom(ssp_i12)
				VR_Obj.Visible = 0
			Next
		End If
		Dim ssp_i13
		If IsArray(VRBackglass) Then
			For ssp_i13 = 0 To UBound(VRBackglass)
				Set VR_Obj = VRBackglass(ssp_i13)
				VR_Obj.visible = 0
			Next
		End If
	End If
...
On Error Resume Next ' Wine VBScript SolCallback fix
...
Sub FlashPWM(idx, light, lightmaps, p)
	...
	Dim x, ssp_i0
	If IsArray(lightmaps) Then
		For ssp_i0 = 0 To UBound(lightmaps)
			Set x = lightmaps(ssp_i0)
			x.color = v : x.opacity = 100.0 / i
		Next
	End If
	light.State = p
...
Sub LeftFlipper_Animate
	...
	Dim ssp_i14
	If IsArray(BP_LFlipper) Then
		For ssp_i14 = 0 To UBound(BP_LFlipper)
			Set LM = BP_LFlipper(ssp_i14)
			LM.Rotz = lfa : LM.opacity = 100 - v
		Next
	End If
	Dim ssp_i15
	If IsArray(BP_LFlipperU) Then
		For ssp_i15 = 0 To UBound(BP_LFlipperU)
			Set LM = BP_LFlipperU(ssp_i15)
			LM.Rotz = lfa : LM.opacity = v
		Next
	End If
...
Sub LeftFlipper1_Animate
	...
	Dim ssp_i16
	If IsArray(BP_LFlipper1) Then
		For ssp_i16 = 0 To UBound(BP_LFlipper1)
			Set LM = BP_LFlipper1(ssp_i16)
			LM.Rotz = lfa1 : LM.Opacity = 100 - v
		Next
	End If
	Dim ssp_i17
	If IsArray(BP_LFlipper1U) Then
		For ssp_i17 = 0 To UBound(BP_LFlipper1U)
			Set LM = BP_LFlipper1U(ssp_i17)
			LM.Rotz = lfa1 : LM.Opacity = v
		Next
	End If
...
Sub RightFlipper_Animate
	...
	Dim ssp_i18
	If IsArray(BP_RFlipper) Then
		For ssp_i18 = 0 To UBound(BP_RFlipper)
			Set LM = BP_RFlipper(ssp_i18)
			LM.Rotz = rfa : LM.Opacity = 100 - v
		Next
	End If
	Dim ssp_i19
	If IsArray(BP_RFlipperU) Then
		For ssp_i19 = 0 To UBound(BP_RFlipperU)
			Set LM = BP_RFlipperU(ssp_i19)
			LM.Rotz = rfa : LM.Opacity = v
		Next
	End If
...
Sub RightSlingShot_Timer
    ...
    End Select
	Dim ssp_i20
	If IsArray(BP_RSling1) Then
		For ssp_i20 = 0 To UBound(BP_RSling1)
			Set x = BP_RSling1(ssp_i20)
			x.Visible = x1
		Next
	End If
	Dim ssp_i21
	If IsArray(BP_RSling2) Then
		For ssp_i21 = 0 To UBound(BP_RSling2)
			Set x = BP_RSling2(ssp_i21)
			x.Visible = x2
		Next
	End If
	Dim ssp_i22
	If IsArray(BP_REMK) Then
		For ssp_i22 = 0 To UBound(BP_REMK)
			Set x = BP_REMK(ssp_i22)
			x.transx = y
		Next
	End If
    RStep = RStep + 1
...
Sub LeftSlingShot_Timer
    ...
    End Select
	Dim ssp_i23
	If IsArray(BP_LSling1) Then
		For ssp_i23 = 0 To UBound(BP_LSling1)
			Set x = BP_LSling1(ssp_i23)
			x.Visible = x1
		Next
	End If
	Dim ssp_i24
	If IsArray(BP_LSling2) Then
		For ssp_i24 = 0 To UBound(BP_LSling2)
			Set x = BP_LSling2(ssp_i24)
			x.Visible = x2
		Next
	End If
	Dim ssp_i25
	If IsArray(BP_LEMK) Then
		For ssp_i25 = 0 To UBound(BP_LEMK)
			Set x = BP_LEMK(ssp_i25)
			x.transx = y
		Next
	End If
    LStep = LStep + 1
...
Sub TopSlingShot_Timer
    ...
    End Select
	Dim ssp_i26
	If IsArray(BP_MSling1) Then
		For ssp_i26 = 0 To UBound(BP_MSling1)
			Set x = BP_MSling1(ssp_i26)
			x.Visible = x1
		Next
	End If
	Dim ssp_i27
	If IsArray(BP_MSling2) Then
		For ssp_i27 = 0 To UBound(BP_MSling2)
			Set x = BP_MSling2(ssp_i27)
			x.Visible = x2
		Next
	End If
	Dim ssp_i28
	If IsArray(BP_LEMK1) Then
		For ssp_i28 = 0 To UBound(BP_LEMK1)
			Set x = BP_LEMK1(ssp_i28)
			x.transy = y
		Next
	End If
    TStep = TStep + 1
...
Set ssp_newobj = new DropTarget : Set DT33 = ssp_newobj(sw33, sw33a, BP_sw33, 33, 0, false)
Set ssp_newobj = new DropTarget : Set DT34 = ssp_newobj(sw34, sw34a, BP_sw34, 34, 0, false)
Set ssp_newobj = new DropTarget : Set DT35 = ssp_newobj(sw35, sw35a, BP_sw35, 35, 0, false)
Set ssp_newobj = new DropTarget : Set DT36 = ssp_newobj(sw36, sw36a, BP_sw36, 36, 0, false)
Set ssp_newobj = new DropTarget : Set DT57 = ssp_newobj(sw57, sw57a, BP_sw57, 57, 0, false)
Set ssp_newobj = new DropTarget : Set DT59 = ssp_newobj(sw59, sw59a, BP_sw59, 59, 0, false)
Dim DTArray
...
Sub DoDTAnim()
	If Not IsArray(DTArray) Then Exit Sub
	Dim i
	For i=0 to Ubound(DTArray)
...
'			BOT = GetBalls
	...
	Dim x, ssp_i1
	If IsArray(lightmaps) Then
		For ssp_i1 = 0 To UBound(lightmaps)
			Set x = lightmaps(ssp_i1)
			x.transz=prim.transz: x.rotx=prim.rotx: x.roty=prim.roty
		Next
	End If
End Function
...
Set ssp_newobj = new StandupTarget : Set ST17 = ssp_newobj(sw17, BP_sw17, 17, 0)
Set ssp_newobj = new StandupTarget : Set ST18 = ssp_newobj(sw18, BP_sw18, 18, 0)
Set ssp_newobj = new StandupTarget : Set ST19 = ssp_newobj(sw19, BP_sw19, 19, 0)
Set ssp_newobj = new StandupTarget : Set ST20 = ssp_newobj(sw20, BP_sw20, 20, 0)
...
Function STAnimate(primary, lightmaps, switch,  animate)
	...
	Dim x, ssp_i2
	If IsArray(lightmaps) Then
		For ssp_i2 = 0 To UBound(lightmaps)
			Set x = lightmaps(ssp_i2)
			x.transy=prim.transy
		Next
	End If
End Function
