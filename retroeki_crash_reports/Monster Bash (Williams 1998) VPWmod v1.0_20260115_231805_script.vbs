...
' Safe UBound wrapper - returns -1 for uninitialized/empty arrays
...
' Safe 2D array read: arr(idx1)(idx2)
Function VPX_Get2DArray(arr, idx1, idx2)
    On Error Resume Next
    VPX_Get2DArray = Empty
    Dim temp : temp = arr(idx1)
    VPX_Get2DArray = temp(idx2)
    On Error Goto 0
End Function

' Safe 2D array write: arr(idx1)(idx2) = value
Sub VPX_Set2DArray(ByRef arr, idx1, idx2, value)
    On Error Resume Next
    Dim temp : temp = arr(idx1)
    temp(idx2) = value
    arr(idx1) = temp
...
' DTArray helpers (if using drop targets)
Function DTGet(arr, idx, prop)
    On Error Resume Next
    DTGet = Empty
    Dim obj : Set obj = arr(idx)
    DTGet = obj(prop)
    On Error Goto 0
End Function

Function DTGetObj(arr, idx, prop)
    On Error Resume Next
    Set DTGetObj = Nothing
    Dim obj : Set obj = arr(idx)
    Set DTGetObj = obj(prop)
    On Error Goto 0
End Function

Sub DTSet(ByRef arr, idx, prop, value)
    On Error Resume Next
    Dim obj : Set obj = arr(idx)
    obj(prop) = value
    On Error Goto 0
...
' STArray helpers (if using standup targets)
Function STGet(arr, idx, prop)
    On Error Resume Next
    STGet = Empty
    Dim obj : Set obj = arr(idx)
    STGet = obj(prop)
...
Function STGetObj(arr, idx, prop)
    On Error Resume Next
    Set STGetObj = Nothing
    Dim obj : Set obj = arr(idx)
    Set STGetObj = obj(prop)
    On Error Goto 0
End Function

Sub STSet(ByRef arr, idx, prop, value)
    On Error Resume Next
    Dim obj : Set obj = arr(idx)
    obj(prop) = value
    On Error Goto 0
...
Sub Table1_KeyDown(ByVal Keycode)
	...
	If keycode = PlungerKey Then Controller.Switch(11) = 1: soundStartButton()
...
Class FlipperPolarity
	...
	Public Sub Class_Initialize 
		...
		Enabled = True : TimeDelay = 50 : LR = 1:  dim x : for x = 0 to VPX_SafeUBound(balls) : balls(x) = Empty : set Balldata(x) = new SpoofBall : next 
	End Sub
	...
	Public Sub Report(aChooseArray) 	'debug, reports all coords in tbPL.text
		...
		End Select
		dim str, x : for x = 0 to VPX_SafeUBound(a1) : str = str & aChooseArray & " x: " & round(a1(x),4) & ", " & round(a2(x),4) & vbnewline : next
		tbpl.text = str
	End Sub
	
	Public Sub AddBall(aBall) : dim x : for x = 0 to VPX_SafeUBound(balls) : if IsEmpty(balls(x)) then set balls(x) = aBall : exit sub :end if : Next  : End Sub

	Private Sub RemoveBall(aBall)
		dim x : for x = 0 to VPX_SafeUBound(balls)
	...
	Public Sub PolarityCorrect(aBall)
		if FlipperOn() then 
			...
			for x = 0 to VPX_SafeUBound(Balls)
...
Sub ShuffleArray(ByRef aArray, byVal offset) 'shuffle 1d array
	...
	redim a(VPX_SafeUBound(aArray) )
	for x = 0 to VPX_SafeUBound(aArray)	'Shuffle objects in a temp array
...
Function LinearEnvelope(xInput, xKeyFrame, yLvl)
	dim y 'Y output
	dim L 'Line
	dim ii : for ii = 1 to VPX_SafeUBound(xKeyFrame)	'find active line
		if xInput <= xKeyFrame(ii) then L = ii : exit for : end if
	Next
	if xInput > xKeyFrame(VPX_SafeUBound(xKeyFrame) ) then L = VPX_SafeUBound(xKeyFrame)	'catch line overrun
	Y = pSlope(xInput, xKeyFrame(L-1), yLvl(L-1), xKeyFrame(L), yLvl(L) )

	if xInput <= xKeyFrame(lBound(xKeyFrame) ) then Y = yLvl(lBound(xKeyFrame) ) 	'Clamp lower
	if xInput >= xKeyFrame(VPX_SafeUBound(xKeyFrame) ) then Y = yLvl(VPX_SafeUBound(xKeyFrame) )	'Clamp upper
...
Class Dampener
	...
	public sub Dampen(aBall)
		if threshold then if BallSpeed(aBall) < threshold then exit sub end if
	...
	Public Sub CopyCoef(aObj, aCoef) 'alternative addpoints, copy with coef
		dim x : for x = 0 to VPX_SafeUBound(aObj.ModIn)
	...
	Public Sub Report() 	'debug, reports all coords in tbPL.text
		...
		dim str, x : for x = 0 to VPX_SafeUBound(a1) : str = str & x & ": " & round(a1(x),4) & ", " & round(a2(x),4) & vbnewline : next
		TBPout.text = str
	End Sub
...
Class CoRTracker
	...
	Public Sub Update()	'tracks in-ball-velocity
		...
		if VPX_SafeUBound(ballvel) < highestID then redim ballvel(highestID)	'set bounds
		if VPX_SafeUBound(ballvelx) < highestID then redim ballvelx(highestID)	'set bounds
		if VPX_SafeUBound(ballvely) < highestID then redim ballvely(highestID)	'set bounds
...
Sub STHit(switch)
	...
	'PlayTargetSound
	VPX_Set2DArray STArray, i, 3, VPX_Get2DArray(STCheckHit, Activeball,STArray(i, 0))

	If VPX_Get2DArray(STArray, i, 3) <> 0 Then
		DTBallPhysics Activeball, VPX_Get2DArray(STArray, i, 0).orientation, STMass
	End If
	DoSTAnim
End Sub
...
Function STArrayID(switch)
	Dim i
	For i = 0 to VPX_SafeUBound(STArray) 
		If VPX_Get2DArray(STArray, i, 2) = switch Then STArrayID = i:Exit Function 
	Next
End Function
...
Sub DoSTAnim()
	Dim i
	For i=0 to VPX_SafeUBound(STArray)
		VPX_Set2DArray STArray, i, 3, VPX_Get2DArray(STAnimate, STArray(i, 0),VPX_Get2DArray(STArray, i, 1),VPX_Get2DArray(STArray, i, 2),VPX_Get2DArray(STArray, i, 3))
	Next
End Sub
...
' Import the shadow materials VPX_Get2DArray(file, 3 sets included, you can also export the 3 sets from this table to create the same file)
...
Sub DynamicBSUpdate
	...
	For s = VPX_SafeUBound(BOT) + 1 to tnob
		objrtx1(s).visible = 0
		objrtx2(s).visible = 0
	...
	If VPX_SafeUBound(BOT) < lob Then Exit Sub		'No balls in play, exit

'The Magic happens now
	For s = lob to VPX_SafeUBound(BOT)
		...
		If DynamicBallShadowsOn Then
			If BOT(s).Z < 30 Then 'And BOT(s).Y < (TableHeight - 200) Then 'Or BOT(s).Z > 105 Then		'Defining when and where (on the table) you can have dynamic shadows
				For iii = 0 to numberofsources - 1 
					LSd=DistanceFast((BOT(s).x-VPX_Get2DArray(DSSources, iii, 0)),(BOT(s).y-VPX_Get2DArray(DSSources, iii, 1)))	'Calculating the Linear distance to the Source
					If LSd < falloff And light20.state > 0 Then						    'If the ball is within the falloff range of a light and light is on (we will set numberofsources to 0 when GI is off)
						...
						if currentShadowCount(s) = 1 Then						'1 dynamic shadow source
							...
							objrtx1(s).rotz = VPX_Get2DArray(AnglePP, DSSources(iii, 0), VPX_Get2DArray(DSSources, iii, 1), BOT(s).X, BOT(s).Y) + 90
						...
						Elseif currentShadowCount(s) = 2 Then
							...
							objrtx1(s).rotz = VPX_Get2DArray(AnglePP, DSSources(AnotherSource, 0),VPX_Get2DArray(DSSources, AnotherSource, 1), BOT(s).X, BOT(s).Y) + 90
							ShadowOpacity = (falloff-DistanceFast((BOT(s).x-VPX_Get2DArray(DSSources, AnotherSource, 0)),(BOT(s).y-VPX_Get2DArray(DSSources, AnotherSource, 1))))/falloff
							objrtx1(s).size_y = Wideness*ShadowOpacity+Thinness
							UpdateMaterial currentMat,1,0,0,0,0,0,ShadowOpacity*DynamicBSFactor^3,RGB(0,0,0),0,0,False,True,0,0,0,0

							currentMat = objrtx2(s).material
							objrtx2(s).visible = 1 : objrtx2(s).X = BOT(s).X : objrtx2(s).Y = BOT(s).Y + fovY
							objrtx2(s).rotz = VPX_Get2DArray(AnglePP, DSSources(iii, 0), VPX_Get2DArray(DSSources, iii, 1), BOT(s).X, BOT(s).Y) + 90
