...
class Algorithm {
protected:
    mutable const ABGR8* __restrict m_srcBuffer;
    ...
    void init(const ABGR8* __restrict srcBuffer, int srcWidth, int srcHeight) const {
...
public:
    ...
    void run(const ABGR8* const __restrict srcBuffer, ABGR8* const __restrict dst, int srcWidth, int srcHeight) const {
        ...
        for (int srcY = 0; srcY < srcHeight; ++srcY) {
            ...
            for (srcX = 0; srcX < srcWidth; ++srcX) {
                ...
                if (((A ^ E) | (B ^ E) | (C ^ E) | (D ^ E) | (F ^ E) | (G ^ E) | (H ^ E) | (I ^ E)) != 0) {
                    const ABGR8 P = src(srcX, srcY - 2), S = src(srcX, srcY + 2);
                    const ABGR8 Bl = luma(B), Dl = luma(D), El = luma(E), Fl = luma(F), Hl = luma(H);
                ...
                ABGR8* __restrict dstPacked = (ABGR8*)dst + dstIndex;
