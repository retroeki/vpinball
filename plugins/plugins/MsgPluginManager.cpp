   ...
   const std::string& version, const std::string& link, msgpi_load_plugin loadPlugin, msgpi_unload_plugin unloadPlugin)
{
   ...
   // Check if plugin with this ID already exists - return existing one to prevent duplicates
   for (const auto& existing : m_plugins) {
      if (StrCompareNoCase(existing->m_id, id)) {
         PLOGI << "Plugin " << id << " already registered, reusing existing instance";
         return existing;
      }
   }
