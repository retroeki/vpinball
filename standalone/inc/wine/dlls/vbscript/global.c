...
#else
    // Standalone: silently return positive response based on button type
    if(res) {
        V_VT(res) = VT_I2;
        // Return appropriate positive response based on MsgBox type
        // MB_YESNO=4, MB_YESNOCANCEL=3 expect IDYES; others expect IDOK
        if((type & 0x7) == 3 || (type & 0x7) == 4)
            V_I2(res) = IDYES;  // Return Yes for Yes/No dialogs
        else
            V_I2(res) = IDOK;   // Return OK for OK dialogs
    }
