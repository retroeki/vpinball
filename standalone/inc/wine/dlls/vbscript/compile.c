...
static HRESULT compile_call_expression(compile_ctx_t *ctx, call_expression_t *expr, BOOL ret_val)
{
    ...
    /* Wine VBScript Bug: (new ClassName)(args) pattern doesn't work properly.
     * DISPID_VALUE invocation on script-defined classes fails.
     * Workaround: Transform to explicit Init method call.
     * This is a common VPX pattern where classes have an Init default method.
     */
    if(call->type == EXPR_NEW) {
        /* Compile: new ClassName -> creates object on stack */
        hres = compile_expression(ctx, call);
        if(FAILED(hres))
            return hres;
        /* Call Init method instead of default (DISPID_VALUE) */
        return push_instr_bstr_uint(ctx, ret_val ? OP_mcall : OP_mcallv, L"Init", arg_cnt);
    }
#endif
