...
/* Simple wide string helpers for recursion tracking */
static inline int wstr_cmp(const WCHAR *s1, const WCHAR *s2)
{
    while(*s1 && *s1 == *s2) { s1++; s2++; }
    return *s1 - *s2;
}

static inline void wstr_cpy(WCHAR *dst, const WCHAR *src, int max_len)
{
    int i;
...
HRESULT exec_script(script_ctx_t *ctx, BOOL extern_caller, function_t *func, vbdisp_t *vbthis, DISPPARAMS *dp, VARIANT *res)
{
    ...
    /*
     * DEBUG LOGGING FOR ANDROID/WINE VBSCRIPT (Added January 2026)
     * 
     * Purpose: Help diagnose infinite recursion crashes in VBScript execution.
     * 
     * How it works:
     * - Tracks recursion depth of exec_script calls
     * - Logs function name when depth exceeds 50 (early warning)
    ...
    static WCHAR last_func_name[256] = {0};
    static int same_func_count = 0;
    ...
    if (func->name && wstr_cmp(func->name, last_func_name) == 0) {
        ...
        if (func->name) {
            wstr_cpy(last_func_name, func->name, 256);
        }
        same_func_count = 1;
    }
